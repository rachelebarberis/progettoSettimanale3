...
console.log(splitMe('Speriamo in bene'));
/* ESERCIZIO 4
  Crea una funzione chiamata "deleteOne" che riceve una stringa e un booleano come parametri.
  Se il valore booleano è true la funzione deve ritornare la stringa senza il primo carattere, altrimenti la deve ritornare senza l'ultimo.
*/
function deleteOne(name, remove) {
  if (remove) {
    return name.slice(1);
  } else {
...
console.log(isThisAnEmail('rachele.barberis.12gmail.com'));
...
console.log(isThisAnEmail('rachele.barberis.12@gmail.com'));
/* ESERCIZIO 7
...
function whatDayIsIt() {
  ...
  const sett = today.getDay();
  return daysWeek[sett];
}
...
function isTodayMyBirthday() {
  const myBirthday= new Date('2001-02-03');
  const today= new Date();

  if (today.getDay() === myBirthday.getDay() && today.getMonth()===myBirthday.getMonth()) {
    return true 
    } else {
      return false
    }
...
const newestMovie = (array) => {
  ...
  array.forEach((element)=> {
    let currentYear =parseInt(element.Year)
...
const sumAllTheYears = (array) => {
  return array.reduce((totale,movie) =>totale + parseInt(movie.Year),0);
}
